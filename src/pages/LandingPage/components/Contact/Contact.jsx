...
import { validatePhone, validateEmail, validateRequired } from '@/utils/formValidation';
...
const Contact = () => {
  ...
  const handleLocationClick = (branch) => {
    ...
    if (branch?.lat && branch?.lng) {
      const addressTitle =
        branch.contactInfo?.[branch.contactInfo.length - 1]?.text || branch.title;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const errorResult = handleApiError(error, (errorInfo) => {
        setSubmitStatus({
          success: false,
          message: errorInfo.message || 'خطا در ارسال پیام. لطفاً دوباره تلاش کنید.',
        });
      });
  ...
  return (
    <section id="contact" className="contact section">
        ...
        <div className="contact-layout">
          <div className="contact-card-grid">
            {branches.map((branch, index) => (
              <BranchCard key={index} branchData={branch} onLocationClick={handleLocationClick} />
            ))}
          </div>

          <div className="contact-form-shell">
            <div className="contact-form-card-wrapper">
              <div className="contact-form-card" style={{ borderTop: "none", boxShadow: "none" }} data-no-top-bar="true">
                <form onSubmit={handleSubmit} className="contact-form">
                  ...
                  {submitStatus && (
                    <div className={`contact-status ${submitStatus.success ? "is-success" : "is-error"}`}>
      ...
      {selectedBranch && (
        <div className="contact-location-modal" role="dialog" aria-modal="true" aria-label={`آدرس ${selectedBranch.title}`}>
          <div className="contact-location-modal__content">
            <button className="contact-location-modal__close" onClick={closeModal} aria-label="بستن">
              ×
            </button>
            <h4>{selectedBranch.title}</h4>
            <p className="contact-location-modal__address">
              {selectedBranch.contactInfo[selectedBranch.contactInfo.length - 1]?.text}
            </p>
            {mapUrl && (
            ...
            {mapUrl && (
              <div className="contact-location-modal__meta">
                <div className="contact-location-modal__meta-row">
                  <span className="contact-location-modal__meta-label">لینک نقشه:</span>
                  <a href={mapUrl} target="_blank" rel="noreferrer">
                    {mapUrl}
                  </a>
                </div>
                <div className="contact-location-modal__meta-row">
                  <span className="contact-location-modal__meta-label">مختصات:</span>
                  <span>
                    {Number(selectedBranch.lat).toFixed(6)}, {Number(selectedBranch.lng).toFixed(6)}
                  </span>
                </div>
              </div>
            )}
